...
exports.handler = async function(event) {
  ...
  const { sl_avatar_key, clan_name, clan_key, motto, summary, primary_color, secondary_color } = body;
  if (!sl_avatar_key || !clan_name || !clan_key)
    return json(400, { success: false, message: "sl_avatar_key, clan_name and clan_key required." });
  const { data, error } = await supabase.rpc("found_major_clan", {
