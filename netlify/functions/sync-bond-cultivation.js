...
const SYNCABLE_DISPLAY_STATES = new Set(["active"]);
const INFO_ONLY_DISPLAY_STATES = new Set([
  "awaiting_partner_completion",
  "pair_completed"
]);
...
function detectCultivationState(member) {
  ...
  return {
    is_active: isActive,
    cultivation_status: status || "idle",
...
function getSyncConflictMessage(displayState, volumeNumber, bookNumber) {
...
function getSyncAction(postDisplayState, autoPaused) {
  if (autoPaused) return "auto_paused_not_cultivating";

  const state = safeLower(postDisplayState);

  if (state === "awaiting_partner_completion") return "awaiting_partner_completion";
  if (state === "pair_completed") return "pair_completed";
  if (state === "active") return "synced";
...
async function pauseBondBook({
  ...
  const { data, error } = await partnerSupabase.rpc(
    "pause_partner_bond_member_book",
    {
      p_partnership_uuid: partnershipUuid,
      p_member_id: memberId,
      p_bond_volume_number: volumeNumber,
  ...
  if (error) {
    throw new Error(`Failed to pause bond book: ${error.message}`);
  }

  return normalizeRpcRow(data);
}

async function syncBondBook({
  partnershipUuid,
  memberId,
  volumeNumber,
  bookNumber
  ...
  const { data, error } = await partnerSupabase.rpc(
    "sync_partner_bond_member_book_progress",
    {
      p_partnership_uuid: partnershipUuid,
      p_member_id: memberId,
      p_bond_volume_number: volumeNumber,
  ...
  if (error) {
    throw new Error(`Failed to sync bond book: ${error.message}`);
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const volumeNumber = parsePositiveInteger(requestSource.volume_number);
    const bookNumber = parsePositiveInteger(requestSource.book_number);
    ...
    if (!sessionToken) {
      return buildResponse(401, {
        success: false,
        message: "Missing session cookie. Authentication required."
      });
    }
    ...
    if (INFO_ONLY_DISPLAY_STATES.has(preDisplayState)) {
      const [snapshot, bookRows, catalogBook] = await Promise.all([
        loadSnapshot({
          partnershipUuid,
          memberId,
          volumeNumber,
      ...
      return buildResponse(200, {
        ...
        action: safeLower(preDisplayState),
        message: getInfoOnlyMessage(preDisplayState, volumeNumber, bookNumber),
        ...
        book: {
          ...
          pair: {
            ...
            pair_completed: preDisplayState === "pair_completed"
          }
        },
    ...
    if (!SYNCABLE_DISPLAY_STATES.has(preDisplayState)) {
      return buildResponse(409, {
        success: false,
        message: getSyncConflictMessage(preDisplayState, volumeNumber, bookNumber),
        display_state: preDisplayState,
        partnership: {
          partnership_uuid: partnershipUuid,
          partnership_id: parsePositiveInteger(activePartnership.partnership_id) || null,
    ...
    if (!memberCultivation.is_active) {
      await pauseBondBook({
        partnershipUuid,
        memberId,
        volumeNumber,
    ...
    const action = getSyncAction(postDisplayState, autoPaused);
    const message = getSyncMessage(postDisplayState, volumeNumber, bookNumber, autoPaused);
    ...
    console.error("sync-bond-cultivation error:", error);

    return buildResponse(500, {
      success: false,
      message: "Failed to sync bond cultivation.",
