...
function inferErrorStatus(message) {
  ...
  if (!text) return 500;
  if (text.includes("missing required")) return 400;
  if (text.includes("clan not found")) return 404;
  if (text.includes("member not found")) return 404;
  if (text.includes("already has an active clan membership")) return 409;
  if (text.includes("already has a pending clan petition")) return 409;
  if (text.includes("not joinable")) return 409;
  if (text.includes("closed to new petitions")) return 409;
  ...
  try {
    ...
    if (!member) {
      return buildResponse(404, {
        success: false,
        message: "Cultivation member not found. Register your character first before petitioning to join a clan."
      });
    }
    ...
    const { data, error } = await supabase.rpc("create_clan_join_request", {
      p_clan_key: clan_key,
    ...
    if (error) {
      console.error("create-clan-join-request rpc error:", error);

      const statusCode = inferErrorStatus(error.message);

      return buildResponse(statusCode, {
        success: false,
        message: error.message || "Failed to create clan join request."
      });
    }

    const requestRow = firstRow(data);

    let latestPetition = null;
    try {
      latestPetition = await loadLatestPetition(sl_avatar_key, sl_username);
    } catch (petitionError) {
      console.error("create-clan-join-request petition reload error:", petitionError);
    }

    return buildResponse(200, {
      success: true,
      message: "Clan petition submitted successfully.",
      request: requestRow
        ? {
            request_id: requestRow.request_id,
            clan_id: requestRow.clan_id,
            clan_key: cleanText(requestRow.clan_key),
            request_status: cleanText(requestRow.request_status)
          }
        : null,
    ...
    console.error("create-clan-join-request error:", error);

    return buildResponse(500, {
      success: false,
      message: "Failed to create clan join request.",
