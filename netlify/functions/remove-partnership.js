...
function canRemovePartnership(partnership, currentAvatarKey) {
  ...
  return (
    sameValue(currentAvatarKey, requesterAvatarKey) ||
    sameValue(currentAvatarKey, recipientAvatarKey)
  );
}

function buildRemoveMessage(previousStatus) {
  if (previousStatus === "active" || previousStatus === "accepted") {
    return "Partnership removed successfully";
  }

  if (previousStatus === "pending") {
    return "Pending partner request removed successfully";
  }

  return "Partnership removed";
}

async function clearSelectedPartnershipsByUuid(partnershipUuid) {
  const cleanUuid = safeUuid(partnershipUuid);
  if (!cleanUuid) return 0;

  const { data, error } = await supabase
    .schema("partner")
    .from("member_selected_partnerships")
    .delete()
    .eq("selected_partnership_id", cleanUuid)
    .select("member_id");

  if (error) {
    throw new Error(`Failed to clear selected partnership rows: ${error.message}`);
  }
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (!["pending", "active", "accepted"].includes(currentStatus)) {
      return json(409, {
        success: false,
        message: "Only pending or active partnerships can be removed",
        partnership: {
          partnership_id: partnershipId,
    ...
    if (!canRemovePartnership(partnership, currentAvatarKey)) {
      return json(403, {
        success: false,
        message: "You do not have permission to remove this partnership"
      });
    }
    ...
    let selection_cleared_count = 0;

    if (updatedPartnershipUuid) {
      try {
        selection_cleared_count = await clearSelectedPartnershipsByUuid(updatedPartnershipUuid);
      } catch (selectionError) {
        console.error("remove-partnership selection clear warning:", selectionError.message);
      }
    }

    return json(200, {
      success: true,
      message: buildRemoveMessage(currentStatus),
      lookup_source,
      selection_cleared: selection_cleared_count > 0,
      ...
      partnership: {
        ...
        removed_at: updatedRow.removed_at || null,
