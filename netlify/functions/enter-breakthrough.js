...
async function callV2EnterBreakthrough(slAvatarKey) {
  const { data, error } = await breakthroughSupabase.rpc(
    "v2_enter_breakthrough",
    { p_sl_avatar_key: slAvatarKey }
  );
  if (error) throw new Error("v2_enter_breakthrough RPC failed: " + error.message);
...
exports.handler = async function(event) {
  ...
  try {
    ...
    let result;
    try {
      result = await callV2EnterBreakthrough(member.sl_avatar_key);
    } catch (rpcError) {
      console.error("enter-breakthrough v2_enter_breakthrough error:", rpcError);
      return buildResponse(500, { success: false, message: "Failed to enter breakthrough.", error: rpcError.message });
    }

    if (!result) return buildResponse(500, { success: false, message: "v2_enter_breakthrough returned no result." });

    if (!result.success) {
      const errorCode = safeText(result.error_code);
      const message = safeText(result.message, "Enter breakthrough failed.");
      const statusMap = {
        member_not_found: 404, avatar_key_required: 400, no_breakthrough_available: 409,
        stage_not_complete: 409, breakthrough_gate_closed: 409, stage_needs_repair: 409,
        cooldown_active: 409, breakthrough_already_exists: 409
      };
      ...
      if (result.repair_cp_cost !== undefined) responseBody.repair_cp_cost = result.repair_cp_cost;
    ...
    return buildResponse(200, {
      success: true,
      message: safeText(result.message) || "Breakthrough entered. Proceed to begin the timer.",
      stage_state_id: safeText(result.stage_state_id) || null,
      breakthrough_state_id: safeText(result.breakthrough_state_id) || null,
      lifecycle_status: safeText(result.lifecycle_status, "pending"),
      next_action: safeText(result.next_action) || "begin_breakthrough",
    ...
    console.error("enter-breakthrough error:", error);
    return buildResponse(500, { success: false, message: "Failed to enter breakthrough.", error: error.message || "Unknown error." });
  }
};