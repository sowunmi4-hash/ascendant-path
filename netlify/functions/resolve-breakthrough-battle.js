...
//
// Calls breakthrough.v2_resolve_breakthrough_battle(p_sl_avatar_key).
//
// This is the RESOLVE step only.
// It should replace the old V1 resolve logic in this live endpoint.
//
// DB owns all real resolution truth, including things like:
// - whether a live breakthrough exists
// - whether the timer has completed
// - whether the battle can be resolved now
// - success / failure outcome
// - verdict persistence
// - cooldown / repair / damage / progression effects
//
// JS stays thin:
// - accept request
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data, error } = await breakthroughSupabase.rpc(
      "v2_resolve_breakthrough_battle",
      {
    ...
    if (error) {
      return buildResponse(400, {
        success: false,
        message: error.message || "Failed to resolve breakthrough battle."
      });
    }
    ...
    if (data == null) {
      return buildResponse(500, {
        success: false,
        message: "Resolve completed with no payload returned from DB."
      });
    }
    ...
    if (payload?.success === false) {
      return buildResponse(400, {
        ...
        message:
          ...
          "Breakthrough battle resolution failed.",
        ...payload
      });
    }
    ...
    return buildResponse(200, {
      ...
      message:
        ...
        payload?.detail ||
        payload?.status_message ||
        "Breakthrough battle resolved successfully.",
